...
| GET    | `/api/settings`                               | 获取筛选配置                                 |
| POST   | `/api/settings`                               | 更新筛选配置                                 |
...
| `#/settings`    | SettingsPage    | 主题色 / 暗色模式 / 关于信息 |
...
- 语言偏好、暗色模式、主题色选择均持久化到 localStorage
- 玩家导入筛选配置持久化到数据库 `settings` 表（key-value 结构），支持白名单/黑名单/最小时长三种筛选方式
- 白名单与黑名单互斥：添加到一方时自动从另一方移除；白名单不为空时仅导入白名单玩家
- 玩家筛选器使用黄金角分布（137.508°）算法生成高区分度颜色
- ECharts 按需引入：CanvasRenderer、LineChart、BarChart、GridComponent、TooltipComponent、LegendComponent、DataZoomComponent
