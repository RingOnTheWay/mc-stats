      ...
      <div class="relative">
        <div class="flex items-center gap-4 mb-6">
          <div class="w-12 h-12 bg-gradient-to-br from-brand/20 to-brand/10 rounded-xl flex items-center justify-center">
            <Swords class="w-6 h-6 text-brand" />
          </div>
          <h3 class="text-lg font-semibold text-slate-800">{{ category === 'killed' ? t('battle.killed') : t('battle.killed_by') }}</h3>
        </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-slate-50/80">
                ...
                <th class="px-4 py-3 text-left text-sm font-medium text-slate-600">{{ t('common.name') }}</th>
                <th class="px-4 py-3 text-right text-sm font-medium text-slate-600">{{ t('common.count') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(item, i) in topMobs" :key="item.key" class="border-t border-slate-100 hover:bg-slate-50/50 transition-colors">
                <td class="px-4 py-3">
                  <span
