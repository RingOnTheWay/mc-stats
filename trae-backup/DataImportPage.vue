  ...
  <div class="space-y-6">
        ...
        <div class="space-y-4">
          <div>
            ...
            <div class="flex gap-2">
              ...
              <button class="px-4 py-3 bg-brand/10 hover:bg-brand/20 text-brand rounded-xl transition-all flex items-center gap-2" @click="openFolderBrowser('scan')">
        ...
        <div class="space-y-4">
          <div>
            ...
            <div class="flex gap-2">
              ...
              <button class="px-4 py-3 bg-brand/10 hover:bg-brand/20 text-brand rounded-xl transition-all flex items-center gap-2" @click="openFolderBrowser('batch')">
                <FolderOpen class="w-4 h-4" />
              </button>
            </div>
          </div>
    ...
    <div v-if="activeTab === 'delete'" class="space-y-6">
      ...
      <div class="bg-white/70 backdrop-blur-sm rounded-2xl p-8 border border-white/80 shadow-sm hover:shadow-lg transition-all duration-300 group">
        <div class="flex items-center gap-4 mb-6">
          ...
          <h3 class="text-lg font-semibold text-slate-800">{{ t('dataManage.deleteAll') }}</h3>
        </div>

        <div class="space-y-4">
          <p class="text-sm text-slate-500">{{ t('dataManage.deleteAllHint') }}</p>
          <button
          ...
          >
            ...
            <Play v-else class="w-4 h-4" />
            {{ deleteAllLoading ? t('common.loading') : t('dataManage.execute') }}
          </button>
        </div>

        <div v-if="deleteAllError" class="mt-4 p-4 bg-red-50 border border-red-100 rounded-xl text-sm text-red-700">{{ deleteAllError }}</div>
        <div v-if="deleteAllResult" class="mt-4 p-4 bg-emerald-50 border border-emerald-100 rounded-xl">
          <div class="font-semibold text-emerald-800 text-sm mb-2">{{ t('dataManage.deleteAllSuccess') }}</div>
          <div class="flex flex-wrap gap-3 text-xs text-emerald-700">
    ...
    <Teleport to="body">
      <div v-if="showFolderBrowser" class="global-loading-overlay" @click.self="showFolderBrowser = false">
        <div class="folder-browser-card">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 bg-gradient-to-br from-brand/20 to-brand/10 rounded-xl flex items-center justify-center">
                <HardDrive class="w-5 h-5 text-brand" />
              </div>
              <h3 class="text-lg font-semibold text-slate-800">{{ t('dataManage.selectFolder') }}</h3>
            </div>
            <button class="p-2 hover:bg-slate-100 rounded-lg transition-all text-slate-400 hover:text-slate-600" @click="showFolderBrowser = false">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
          </div>

          <div class="mb-3 px-3 py-2 bg-slate-50 rounded-lg text-xs text-slate-500 font-mono break-all">
            {{ folderBrowserPath || t('dataManage.selectDrive') }}
          </div>

          <div v-if="folderBrowserError" class="mb-3 px-3 py-2 bg-red-50 rounded-lg text-xs text-red-600">{{ folderBrowserError }}</div>
          ...
          <div v-else class="max-h-72 overflow-y-auto space-y-0.5">
            <button
              ...
              class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-slate-500 hover:bg-slate-50 transition-all"
            ...
            <button
              ...
              :class="dir.accessible ? 'text-slate-700 hover:bg-brand/5 hover:text-brand' : 'text-slate-300 cursor-not-allowed'"
              @click="dir.accessible && browseTo(dir.path)"
            >
              <Lock v-if="!dir.accessible" class="w-4 h-4 text-slate-300" />
              <Folder v-else class="w-4 h-4 text-brand/60" />
              <span>{{ dir.name }}</span>
            </button>
          </div>
          ...
          <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-end">
            <div class="flex gap-2">
              <button class="px-4 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-lg transition-all" @click="showFolderBrowser = false">{{ t('dataManage.cancel') }}</button>
              <button
